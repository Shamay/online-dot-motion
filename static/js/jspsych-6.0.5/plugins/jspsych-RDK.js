...
jsPsych.plugins["RDK"] = (function() {
	...
	plugin.info = {
	    ...
	    parameters: {
		    choices: {
		      ...
		      default: [],
		      array: true,
		      description: "The valid keys that the subject can press to indicate a response. If left unspecified, any key is a valid key"
		    },
		    correct_choice: {
		      ...
		      default: undefined,
		      array: true,
		      description: "The keys that are considered the correct response for that particular trial"
		    },
		    trial_duration: {
		      type: jsPsych.plugins.parameterType.INT,
		      pretty_name: "Trial duration",
		      default: 1500,
		      description: "The amount of time that the stimulus is displayed on the screen in ms. If -1, the stimulus will be displayed until a valid response"
		    },
		    response_ends_trial: {
		      type: jsPsych.plugins.parameterType.BOOL,
		      pretty_name: "Response ends trial",
		      default: true,
		      description: "If true, then the subject's response will end the trial. If false, the stimuli will be presented for the full trial_duration."
		    },
		    number_of_apertures: {
		      type: jsPsych.plugins.parameterType.INT,
		    ...
		    coherent_direction: {
		      ...
		      default: 'up',
		      description: "The direction of coherent motion"
		    },
				majority_color: {
					type: jsPsych.plugins.parameterType.STRING,
					pretty_name: "Majority dot color",
					default: 'blue',
					description: "The color of the majority of the dots"
				},
				minority_color: {
					type: jsPsych.plugins.parameterType.STRING,
					pretty_name: "Minority dot color",
					default: 'red',
					description: "The color of the minority of the dots,"
				},
		    motion_coherence: {
		      type: jsPsych.plugins.parameterType.FLOAT,
		      pretty_name: "Motion Coherence",
		      default: 0.5,
		      description: "The proportion of dots moving in the coherent direction"
		    },
				opposite_coherence: {
					type: jsPsych.plugins.parameterType.FLOAT,
				...
				color_coherence: {
					...
					default: 0.5,
					description: "The proportion of dots with the majority color"
				},
		    dot_radius: {
		      type: jsPsych.plugins.parameterType.INT,
		      ...
		      description: "The radius of the dots in pixels"
				...
				dot_timeout: {
					type: jsPsych.plugins.parameterType.INT,
					pretty_name: "Dot timeout",
					default: -1,
					description: "The time before the dots disappears in ms. If -1, the dots will be displayed for the full trial_duration."
				},
		    dot_life: {
		      type: jsPsych.plugins.parameterType.INT,
		      pretty_name: "Dot life",
		      default: -1,
		      description: "The number of frames that pass before each dot disappears and reappears somewhere else. If -1, the dot life is infinite."
		    },
		    move_distance: {
		      type: jsPsych.plugins.parameterType.INT,
		      pretty_name: "Move distance",
		      default: 1,
		      description: "The distance in pixels each dot moves per frame (speed of dots)."
		    },
		    aperture_width: {
		      type: jsPsych.plugins.parameterType.INT,
		    ...
		    border_color: {
		      ...
		      pretty_name: "Text",
		      default: "",
		      description: "The text above the stimulus"
		    }
	    }
	 }
	...
	plugin.trial = function(display_element, trial) {
		...
		trial.trial_duration = assignParameterValue(trial.trial_duration, 1500);
		...
		trial.coherent_direction = assignParameterValue(trial.coherent_direction, 'up');
		trial.majority_color = assignParameterValue(trial.majority_color, 'blue');
		trial.minority_color = assignParameterValue(trial.minority_color, 'red');
		trial.motion_coherence = assignParameterValue(trial.motion_coherence, 0.5);
		trial.opposite_coherence = assignParameterValue(trial.opposite_coherence, 0);
		trial.color_coherence = assignParameterValue(trial.color_coherence, 0.5);
		trial.dot_timeout = assignParameterValue(trial.dot_timeout, -1);
		...
		trial.aperture_width = assignParameterValue(trial.aperture_width, 300);
		trial.aperture_height = assignParameterValue(trial.aperture_height, 200);
		...
		trial.aperture_type = assignParameterValue(trial.aperture_type, 'circle');
		trial.reinsert_type = assignParameterValue(trial.reinsert_type, 2);
		trial.aperture_center_x = assignParameterValue(trial.aperture_center_x,(window.innerWidth * 0.85) / 2);
		trial.aperture_center_y = assignParameterValue(trial.aperture_center_y, (window.innerHeight * 0.85) / 2);
		...
		trial.text = assignParameterValue(trial.text, "");


		//For square and circle, set the aperture height == aperture width
		if (apertureType == 'circle' || apertureType == 'square') {
			trial.aperture_height = trial.aperture_width;
		}

		// Transforms text-based directions (up, down, right, left) to degrees so that the code below can use it
		// Returns the direction of the coherentDots in degrees. Starts at 3 o'clock and goes counterclockwise
		// (0 == rightwards, 90 == upwards, 180 == leftwards, 270 == downwards), range 0 - 360
		function stringToDegrees(direction){
			if (direction == 'up'){
				return 90;
		...
		// Returns proportion of dots that are the majority color.
		// Value should not be less than 0.5 or else the majority becomes the minority
		// scaled from range of 0 to 1 to range of 0.5 to 1
		function scaleColorCoherence(coherence){
			return 0.5 + (coherence / 2.0);
		}

		// Resets the aperture center every trial
		// Scale by 0.85 to avoid scrolling bars appearing;
		function resetAperture(inner_measurement){
			return (inner_measurement * 0.85) / 2;
		}
		...
		var coherentDirection = stringToDegrees(trial.coherent_direction); // Transforms the direction of the coherentDots to degrees. Starts at 3 o'clock and goes counterclockwise (0 == rightwards, 90 == upwards, 180 == leftwards, 270 == downwards), range 0 - 360
		var majorityColor = trial.majority_color; //The color of the majority of the dots.
		var minorityColor = trial.minority_color; //The color of the minority of the dots.
		var motionCoherence = trial.motion_coherence; //Proportion of dots to move together, range from 0 to 1
		...
		var colorCoherence = scaleColorCoherence(trial.color_coherence); //Proportion of dots that are the majority color, scaled from 0 to 1 to 0.5 to 1 to maintain majority
		var dotTimeout = trial.dot_timeout; // The time until the dots disappear
		...
		var apertureCenterX = resetAperture(window.innerWidth); // The x-coordinate of center of the aperture on the screen, in pixels
		var apertureCenterY = resetAperture(window.innerHeight); // The y-coordinate of center of the aperture on the screen, in pixels
		var text = trial.text; // the text displayed above the stimulus
		...
		*/
		...
		var canvasWidth = canvas.width = window.innerWidth * 0.85;
		var canvasHeight = canvas.height = window.innerHeight * 0.85;
		...
		var majorityColorArray;
		var minorityColorArray;
		var motionCoherenceArray;
		var colorCoherenceArray;
		...
		//Declare a global dot timeout ID to be initialized below in animateDotMotion function and to be used in after_response function
		var dot_timeoutID;

		//Variable to start the dot timer when the time comes
		...
		function setUpMultipleApertures(){
			...
			majorityColorArray = setParameter(majorityColor);
			minorityColorArray = setParameter(minorityColor);
			motionCoherenceArray = setParameter(motionCoherence);
			colorCoherenceArray = setParameter(colorCoherence);
		...
		function initializeCurrentApertureParameters(){
			...
			majorityColor = majorityColorArray[currentApertureNumber];
			minorityColor = minorityColorArray[currentApertureNumber];
			motionCoherence = motionCoherenceArray[currentApertureNumber];
			colorCoherence = colorCoherenceArray[currentApertureNumber];
			...
			//Calculate the number of coherent, opposite coherent, and incoherent dots for motion
			nCoherentDots = Math.round(nDots * motionCoherence);
			nOppositeCoherentDots = Math.round(nDots * oppositeCoherence);
			nIncoherentDots = nDots - (nCoherentDots + nOppositeCoherentDots);

			//Calculate the number of majority and minority dots for color
			nMajorityColor = Math.round(nDots * colorCoherence);
			nMinorityColor = nDots - nMajorityColor;
		...
		function initializeApertureDimensions() {
			...
			else if (apertureType == 'ellipse' || apertureType == 'rectangle') {
		...
		function makeDotArray() {
			...
			// create an array containing the dot color of each dot, and then shuffle it to randomize the color assignments
			var colorArray = Array(nMajorityColor).fill(majorityColor).concat(Array(nMinorityColor).fill(minorityColor));
			colorArray = shuffleArray(colorArray);

			for (var i = 0; i < nDots; i++) {
				//Initialize a dot to be modified and inserted into the array
				var dot = {
					color: colorArray[i], //dot color
					x: 0, //x coordinate
					y: 0, //y coordinate
      ...
      var dotArray = dotArray2d[currentSetArray[currentApertureNumber]];
			...
			// display text above the stimulus
			if(text != ''){
				ctx.font = "32px Open Sans, Arial, sans-serif";
				ctx.textAlign= "center";
				ctx.fillStyle = "white";
				ctx.fillText(text, canvasWidth/2, canvasHeight/5);
			}
	    ...
	    if(fixationCross === true){
	  	...
	  	if(border === true){
	      	...
	      	if(apertureType === 'circle' || apertureType === 'ellipse'){
	      	...
	      	if(apertureType === 'square' || apertureType === 'rectangle'){
	        		ctx.lineWidth = borderThickness;
	        		ctx.strokeStyle = borderColor;
      ...
      var dotArray = dotArray2d[currentSetArray[currentApertureNumber]];
		...
		function resetLocation(dot) {
			...
			//For square and rectangle
			else if (apertureType == 'square' || apertureType == 'rectangle') {
		...
		function animateDotMotion() {
			// If dot_timeout setting is off, start keyboard listener
			if (trial.dot_timeout == -1){
				 startKeyboardListener();
			 }

			// Dot timeout option function
			...
			if ( (!dotTimerHasStarted) && (trial.dot_timeout > -1) ){
				//If the trial duration is set, then set a timer to count down and call the timeoutDots function when the time is up
